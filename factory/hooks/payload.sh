...
"
  ...
  if [ ! -f "$ALEX_DIR/system/.block_complete" ]; then
    if [ "$has_constitution" = "true" ] && [ ! -f "$ALEX_DIR/system/.block" ]; then
      ...
      echo "private files are finished but the account-instructions proof is not current,"
      echo "resume at Phase 5. Only when $ALEX_DISPLAY/system/.account_instructions_complete"
      echo "matches $ALEX_DISPLAY/system/.account-instructions-required-hash may you run: touch $ALEX_DIR/system/.block_complete"
  ...
  # fresh chat. Changed bytes reopen one bounded refresh, not private onboarding.
  if [ -f "$ALEX_DIR/system/.block_complete" ] \
    ...
    if [ "$account_complete_hash" != "$account_required_hash" ]; then
      ...
      echo "The Alexandria instruction used by ordinary chats is new, changed, or still awaiting proof."
      echo "Read the AI setup section in machine.md first. If this host is unsupported or the Author deferred this step, keep it pending for a useful active-session system action; do not interrupt ordinary work or repeat the ask without changed circumstances. Otherwise give one action: add or replace only the Alexandria text with $ALEX_DISPLAY/system/.account-instructions.md. Preserve every unrelated instruction."
      echo "Then use a brand-new chat for the Phase 5 proof in $ALEX_DISPLAY/system/.block. Only after its reply exactly matches $ALEX_DISPLAY/system/.account-instructions-proof may you copy the hash from $ALEX_DISPLAY/system/.account-instructions-required-hash into $ALEX_DISPLAY/system/.account_instructions_complete."
      echo "Do not rerun the private onboarding or call the account instructions current before that proof."
