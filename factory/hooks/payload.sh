...
"
  ...
  if [ -f "$ALEX_DIR/system/.block_complete" ] \
    ...
    if [ "$account_complete_hash" != "$account_required_hash" ]; then
      ...
      echo "The Alexandria instruction used by ordinary chats has changed."
      echo "Give the Author one action: replace the older Alexandria text in their ordinary AI's account instructions with $ALEX_DISPLAY/system/.account-instructions.md. Preserve every unrelated instruction."
