...
"
  ...
  if [ ! -f "$ALEX_DIR/system/.block_complete" ]; then
    if [ "$has_constitution" = "true" ] && [ ! -f "$ALEX_DIR/system/.block" ]; then
      ...
      echo "private files and supported host wiring are verified, finish Phase 5 or record"
      echo "its honest deferred/unsupported state, then run: touch $ALEX_DIR/system/.block_complete"
      echo "That marker means local readiness only; never forge the separate account-instructions proof."
  ...
  # Account instructions live in the user's AI account, beyond setup's write
  # reach. The completion marker records the exact generated bytes proven in a
  # fresh chat. Missing or changed proof is maintenance for a relevant active
  # session, never a repeated demand in ordinary work or a local-readiness gate.
  if [ -f "$ALEX_DIR/system/.block_complete" ] \
     && [ -s "$ALEX_DIR/system/.account-instructions-required-hash" ]; then
    account_required_hash=$(tr -d '\r\n' < "$ALEX_DIR/system/.account-instructions-required-hash")
    account_complete_hash=""
    if [ -s "$ALEX_DIR/system/.account_instructions_complete" ]; then
      account_complete_hash=$(tr -d '\r\n' < "$ALEX_DIR/system/.account_instructions_complete")
    fi
    if [ "$account_complete_hash" != "$account_required_hash" ]; then
      echo ""
      echo "--- ALEXANDRIA ACCOUNT INSTRUCTIONS UPDATE ---"
      echo "Ordinary-chat instruction loading is unverified or outdated. Do not interrupt ordinary work."
      echo "In a relevant active setup session, follow Phase 5 in $ALEX_DISPLAY/system/.block with $ALEX_DISPLAY/system/.account-instructions.md. Preserve unrelated instructions and honor any deferred or unsupported host recorded in the Author's notepad or machine.md."
      echo "Only saved-field read-back plus the accurate brand-new-chat preference check in Phase 5 permits recording $ALEX_DISPLAY/system/.account-instructions-required-hash in $ALEX_DISPLAY/system/.account_instructions_complete. Never use a diagnostic proof code. Record retrieval is a separate check."
      echo "Do not rerun private onboarding, block the local loop, repeat setup nags or call account coverage verified without evidence."
      echo "--- END ACCOUNT INSTRUCTIONS UPDATE ---"
      echo ""
    fi
  fi
  ...
  if [ "$has_constitution" = "true" ]; then
    ...
    echo "Capture totals have two stages: ready to review and still to process. For any Author-visible count, use the signed installed capture_state.py --summary (or --review for structured output); extraction-only --counts is not the review total. If unavailable, say so rather than reporting zero. Existing approved capture notifications use this same summary; this does not authorize new messages or automation."
    echo "Capture cards use only Link, What, New: clickable source, plain content, and the specific net-new value against the Author's canon (or Nothing substantive; skip). No menu, counter or footer on the card. Skip and delay immediately advance; delay remains open for later; engage pauses for dialogue. Preserve the Author's existing local preferences."
    echo ""
  ...
  if [ "$has_constitution" = "true" ] \
    ...
    if [ -n "$task_cue" ]; then
      ...
      echo "In the first completed assistant reply in this new ordinary task, end with exactly:"
      echo "$task_cue"
      echo "This must be visible assistant text, not a system message, warning, hidden field, or second turn. Never explain it, decorate it, or repeat it in this task. OMIT it during setup/onboarding, install or security review, background work, or an explicit Alexandria start/close skill. If this task already contains that exact question in an earlier assistant reply, do not repeat it. Later replies stay silent unless Alexandria is part of that exchange's actual value."
      echo "--- END ROUTE ---"
      echo ""
    fi
  fi
