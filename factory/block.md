...
- Grok Bot: save `factory/skills/grok-bot.md` into this chat's own skill library as `/a` (and `/alexandria` if the picker lists names). Setup cannot write the Grok Bot box from the Mac. `/a` is an agent-saved Grok Bot workflow with no verified Mac skill dir. If this host's computer connection is down, name the source in one sentence and fall through the capability ladder (the Author's connected GitHub private Alexandria backup of `files/` + `system/`, then a writable attached folder, then writable Drive only if actually authorized); never invent the record, never claim a Mac save. A live chat is not the local-exec daemon: do not tell them to open the app; fully quit with Cmd+Q (closing the window is not enough), `pkill -f local-exec-daemon`, relaunch, then retry. The host's "Execution on Local Computer" control must not be "never allowed"; do not invent a click-path. Do not claim Cursor IDE or Grok CLI hooks exist here. Ordinary Grok Bot chats ask once, text only: `Want me to start an alexandria chat on the side?`
