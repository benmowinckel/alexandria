...
4. **What happens to what they already have.** Nothing. If they already run their own system — a CLAUDE.md, a soul.md, memory files, a structured notes vault — it stays exactly as it is: Alexandria plugs into it, never converts it, never moves it, and after the look they choose how the two fit together (keep theirs, run both, or point ours at theirs — the three modes below). And the whole thing is reversible: everything you build lands in one folder, deleting it is a clean undo (full uninstall at alexandria-library.com/mechanics). Say this even if you see no sign of an existing system — the promise costs nothing and is exactly what a careful person is silently asking.
