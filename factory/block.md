...
- Grok Bot: save `factory/skills/grok-bot.md` into this chat's own skill library as `/a` (and `/alexandria` if the picker lists names). Setup cannot write the Grok Bot box from the Mac. If this host's computer connection is down, say so and stop; never invent the record. Do not claim Cursor IDE or Grok CLI hooks exist here.
