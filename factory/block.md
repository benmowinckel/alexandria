...
- Grok Bot: save `factory/skills/grok-bot.md` into this chat's own skill library as `/a` (and `/alexandria` if the picker lists names). Setup cannot write the Grok Bot box from the Mac. Execution on Local Computer must be allowed; `/a` is an agent-saved Grok Bot workflow with no verified Mac skill dir. If this host's computer connection is down, say so and stop; never invent the record. Do not tell them to open the Grok Bot app if this chat is already live. Chat ≠ local-exec daemon; recovery is fully quit (Cmd+Q), kill leftover `local-exec-daemon`, relaunch, then retry. Also save `factory/skills/grok-bot-close.md` as `a.` / `alexandria-close`. No statusline and no SessionEnd hook: do not invent a footer; this close is the archive. Closing the window is not enough — fully quit means Cmd+Q. Stamp `.last_a` / `.active_a_sessions` on `/a`. Do not skip `.module_guide_seen`. Do not claim Cursor IDE or Grok CLI hooks exist here.
