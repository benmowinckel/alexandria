...
Local setup installs and verifies native hooks first and reads only the personal sources the Author approves. It then has the Author add the generated instruction below the existing instructions in the AI they use for ordinary chats. Setup creates a local random proof line inside `~/alexandria/system/.account-instructions.md`; the Author opens a brand-new chat, asks for that proof, and pastes the reply back to the setup agent. Only an exact proof match records the generated instruction's hash in `.account_instructions_complete`, and only a matching hash allows `.block_complete`. When the signed instruction later changes, the hash mismatch reopens this one replacement-and-proof step without rerunning private onboarding. A statement that the paste happened, the setup chat explaining the behavior, or a matching string in hidden context is not proof of cross-chat persistence.
...
The setup chat first proves the account instructions landed by explaining how the existing ai will now behave; if they did not, it stops and fixes only that. Without opening personal content or changing anything, it identifies the strongest safe home from current connection evidence. It then names the exact sources and destination it can reach and uses one explicit permission to build the fullest accurate first record it can from account memory and accessible past-chat context. It never searches unrelated files, dumps raw chats, invents knowledge or claims an unverified write. A failed path moves down the ladder automatically. The handoff floor says plainly that it was not saved. The ai then starts the highest-value specific thread and saves only confirmed changes. After the private loop works, it states what is ready and gives exactly `https://alexandria-library.com/join` so the person can decide whether they also want a public profile and a way to connect with other people. It does not substitute the homepage, recommend, browse, connect, share, or publish anything.
