...
## Account instructions — after private value, before the optional community

Full local setup installs and verifies native hooks first, reads only the personal sources the Author approves, and shows the first personalized result. Only after that private value works, the installing agent asks which other ai the person uses most and helps them add the same compact Alexandria instructions there. It handles one ai at a time, keeps every existing instruction, and verifies the setting before asking whether they use another. A host with working Alexandria hooks needs no duplicate instruction. The operating rules belong in the host's instruction setting, never its account memory; memory remains content about the person. Record the apps the person confirms, or `none`, in `~/alexandria/system/.other_ai_instructions`. Only after the person is finished with other AIs does the setup show the fixed Library destination. `factory/connect.md` performs this step only for older installs without that marker.
