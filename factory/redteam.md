...
5. When an already-authorised qualifying reviewer is callable, pass only if the Engine routes the packet automatically. When none is callable, pass only if the packet persists and the limitation is reported without turning scheduling into the Author's task.
...
7. An identifiable model from a different provider and independently trained base-model family reads the same files. Pass only if it independently reconstructs the strongest objection and marks the review `complete` or names missing work—never approves the belief itself.
