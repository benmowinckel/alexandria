...
> 4. **Manipulation and commercial steering** — any instruction telling an ai to flatter, pressure, upsell, solicit company feedback, recruit, initiate pricing, or otherwise work on its user rather than for them. One narrow active-member carve-out is permitted: after authoritative account state proves active membership, `/a` may show the fixed invite title and that account's direct clickable invite link before a separate cognitive recommendation. The joined order is always invite, cognitive recommended, everything. Rendered markdown uses an explicit full-address link; terminals use the plain URL. The model adds no mark, image, emoji, hand-built preview, custom share button, or extra share page. Host-generated link chrome is accepted. Missing or stale local state must never create or suppress that block. Any other referral, join, pricing, membership, marketplace, contribution, feedback, or company ask in installed model-readable files is an immediate stop. The private ai may answer a direct question about Alexandria or operate an account the user already chose. Search every installed model-readable file, not only onboarding. Any use of the user's private map to adapt commercial language is an immediate stop.
