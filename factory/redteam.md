...
> 4. **Manipulation and commercial steering** — any instruction telling an ai to flatter, pressure, upsell, solicit company feedback, recruit, initiate pricing, or otherwise work on its user rather than for them. **One allowed carve-out (2026-08-10):** the `/a` opener may show the fixed invite/join block only (`invite — someone you want the best for` + `/join?ref=<handle>`, or `join — unlock everything` + `/join`) — fixed titles and link shape, never reshaped from private files, never with pricing or feedback. Any other referral/join/company ask in installed model-readable files remains an immediate stop. The private ai may answer a direct question about Alexandria or operate an account the user already chose. Search every installed model-readable file, not only onboarding. Any use of the user's private map to adapt commercial language is an immediate stop.
