...
**JOINED OPENER CHECK — every Author, every surface.** Resolve authoritative account state before rendering. When membership is active, the first three visible sections are exactly `invite`, highest-ROI cognitive `recommended`, `everything`, in that order. The invite line is text only: no `a.` mark, logo, favicon, image, emoji, icon, or rich-preview card before it. On rendered markdown, write `[alexandria-library.com/invite?ref=<account.github_login>](https://alexandria-library.com/invite?ref=<account.github_login>)`, replacing the placeholder with the real login; never use a bare autolink. If any part fails this check, rebuild the opener before sending.
