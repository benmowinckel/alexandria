...
**CAPTURE GATE — active means extraction runs, not merely that a count was shown.** Immediately after a healthy classification run `python3 ~/.local/share/alexandria/scripts/capture_state.py --json`, before rendering the opener. If `pending_count` or `raw_count` is nonzero, capture extraction is the session's limiting factor: begin it immediately without asking the Author. Use background workers on disjoint stem lists when the host supports them; otherwise drain sequentially in this session. Narrate progress briefly so the Author is never left staring at silence. Every stem ends with either a rich `saved/<stem>.analysis.md` or a written ledger verdict plus its exact stem in `saved/.drained`; raw files receive the same real payload attempt before any gap is accepted. The opener is forbidden until `python3 ~/.local/share/alexandria/scripts/capture_state.py --gate` exits zero. An Author-paced open item already in `saved/ledger.md` is absorption, not failed extraction, and does not block. A worker report or old count is never proof; the gate reading the files is proof.
