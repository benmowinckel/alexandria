...
**One visible count, two stages.** Use `python3 ~/.local/share/alexandria/scripts/capture_state.py --summary` for every capture count shown to the Author, including existing separately approved text notifications; `--review` provides the same review/processing values as JSON. Review means open ledger items; processing means preserved sources with no recorded disposition, including analysed captures missing a verdict. An analysis alone never means absorbed. Never substitute the extraction-only `--counts`, sidecar totals, cached counts or raw file totals for the review queue. A read failure means count unavailable, never zero. Custom substrates use the same distinction against their mapped source and ledger. This adds no permission to schedule or send a notification.
