...
SCRIPT="$ROOT/factory/scripts/agent_workspace.py"
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
...
printf '%s\n' "$digest" > "$ALEXANDRIA_DIR/system/permissions/agent-workspace-fixture"
python3 "$SCRIPT" enable fixture "$TMP/allowlist.tsv" "$TMP/workspace"
...
if git -C "$TMP/workspace" rev-list --objects --all | grep -q 'secret.md'; then
  echo 'agent workspace test failed: unselected private path entered Git history' >&2
  exit 1
fi

printf 'changed without approval\n' > "$ALEXANDRIA_DIR/files/library/public/everyone.md"
if python3 "$SCRIPT" refresh fixture >/dev/null 2>&1; then
  echo 'agent workspace test failed: changed selected bytes exported without new approval' >&2
  exit 1
fi
...
imported="$(find "$ALEXANDRIA_DIR/files/vault/input/agent/fixture" -type f -name '*.md' -print -quit)"
...
if python3 "$SCRIPT" import fixture >/dev/null 2>&1; then
  echo 'agent workspace test failed: protected context edit was accepted' >&2
  exit 1
fi

printf '../outside\tcontext/nope.md\n' > "$TMP/traversal.tsv"
if python3 "$SCRIPT" plan bad "$TMP/traversal.tsv" >/dev/null 2>&1; then
  echo 'agent workspace test failed: source traversal was accepted' >&2
  exit 1
fi
...
if python3 "$SCRIPT" plan bad "$TMP/symlink.tsv" >/dev/null 2>&1; then
  echo 'agent workspace test failed: source symlink was accepted' >&2
  exit 1
fi
...
if python3 "$SCRIPT" plan bad "$TMP/binary.tsv" >/dev/null 2>&1; then
  echo 'agent workspace test failed: binary source was accepted' >&2
  exit 1
fi

python3 "$SCRIPT" off fixture >/dev/null
if python3 "$SCRIPT" import fixture >/dev/null 2>&1; then
  echo 'agent workspace test failed: off workspace still imported' >&2
  exit 1
fi

echo 'agent workspace tests passed'
