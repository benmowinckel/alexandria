...
class ChatBootstrapTests(unittest.TestCase):
    def test_instruction_is_first_person_additive_and_fits_free_chatgpt(self) -> None:
        ...
        self.assertIn("only on your first reply in that chat", instruction)
        self.assertIn("Never add that line to voice replies", instruction)
        self.assertIn("never add a generic Alexandria footer", instruction)
    ...
    def test_local_installer_keeps_memory_and_adds_exact_folder(self) -> None:
        ...
        self.assertIn("show the state-aware footer only on the first assistant reply in a new text chat", foundation)
        self.assertIn("Voice never reads a generic Alexandria cue aloud", foundation)
