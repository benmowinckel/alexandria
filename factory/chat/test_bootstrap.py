...
class ChatBootstrapTests(unittest.TestCase):
    def test_instruction_is_first_person_additive_and_fits_free_chatgpt(self) -> None:
        ...
        self.assertIn("only each new ordinary chat’s first reply asks “Want me to start an alexandria chat on the side?”", instruction)
        self.assertIn("Never repeat", instruction)
        self.assertIn("On yes, open it with “a” if possible; else tell me how", instruction)
        self.assertIn("Setup routes only at final test", instruction)
        self.assertIn("Later mention it only for a useful read/save", instruction)
        ...
        self.assertIn("On “a.”", instruction)
        self.assertIn("save only confirmed changes", instruction)
