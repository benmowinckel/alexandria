...
class ChatBootstrapTests(unittest.TestCase):
    def test_instruction_is_first_person_additive_and_fits_free_chatgpt(self) -> None:
        ...
        self.assertIn("approved Git (trusted cloud: own branch; else Airlock)", instruction)
        ...
        self.assertIn("`alex_connect_` codes use only `~/alexandria/system/.connect`", instruction)
        self.assertIn("Wait for exact `connect`", instruction)
        self.assertIn("Never browse or expose server text", instruction)
