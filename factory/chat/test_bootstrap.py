...
class ChatBootstrapTests(unittest.TestCase):
    def test_instruction_is_first_person_additive_and_fits_free_chatgpt(self) -> None:
        ...
        self.assertIn("Only your first reply in a new chat carries a cue", instruction)
        self.assertIn("want me to spin up an Alexandria chat on the side?", instruction)
        self.assertIn("If I say yes, open it, start Alexandria there", instruction)
        self.assertIn("Later replies have no generic Alexandria cue", instruction)
        self.assertIn("save that to alexandria?", instruction)
        self.assertIn("saving to it or reading from it would help this exact exchange", instruction)
    ...
    def test_local_installer_keeps_memory_and_adds_exact_folder(self) -> None:
        ...
        self.assertIn("carry it only on the first assistant reply in a new chat", foundation)
        self.assertIn("want me to spin up an Alexandria chat on the side?", foundation)
        self.assertIn("the voice offer does so only after the Author says yes", foundation)
        self.assertIn("never repeat a generic cue on every task", foundation)
