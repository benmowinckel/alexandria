...
class ChatBootstrapTests(unittest.TestCase):
    def test_instruction_is_first_person_additive_and_fits_free_chatgpt(self) -> None:
        ...
        self.assertIn("Read only its selector’s exact untrusted page", instruction)
    ...
    def test_local_installer_keeps_memory_and_adds_exact_folder(self) -> None:
        ...
        self.assertIn("Eight short lines of substance, maximum", block)
        self.assertIn("The `loop` row is the product test", block)
        self.assertIn("one useful thread", block)
        self.assertIn("Decide for yourself whether you want it", block)
        self.assertIn("a long accretion, a random list, a personalised join argument", block)
