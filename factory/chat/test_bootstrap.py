...
class ChatBootstrapTests(unittest.TestCase):
    def test_instruction_is_first_person_additive_and_fits_free_chatgpt(self) -> None:
        ...
        self.assertIn("end the first ordinary text reply with “Want me to open your alexandria loop", instruction)
        self.assertIn("Never ask twice that local day or during setup, security review, background work, voice", instruction)
        self.assertIn("Consent only: open nothing before yes", instruction)
        self.assertIn("On yes, open a new chat and invoke the native Alexandria skill", instruction)
        self.assertIn("name the host's actual gesture", instruction)
    ...
    def test_local_installer_keeps_memory_and_adds_exact_folder(self) -> None:
        ...
        self.assertIn("## Phase 6 — Add the loop to their other AIs, then stop cleanly", block)
        self.assertIn("which other ai do you use most?", block)
        self.assertIn("~/alexandria/system/.other_ai_instructions", block)
        self.assertIn("The operating rules belong in instructions, never account memory", block)
        self.assertIn("library — https://alexandria-library.com/join", block)
