...
class ChatBootstrapTests(unittest.TestCase):
    def test_instruction_is_first_person_additive_and_fits_free_chatgpt(self) -> None:
        ...
        self.assertIn("safest place you can save and reopen", instruction)
        ...
        self.assertIn("Mac/Remote Control: current `~/alexandria` files", instruction)
        self.assertIn("Claude Code Web: my private repo; committed files only; write your own branch", instruction)
        self.assertIn("Other remote ai: Airlock", instruction)
        self.assertNotIn("attached files", instruction)
        self.assertIn("Say Library or Airlock is off only from current permissions, settings or status", instruction)
