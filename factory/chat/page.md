...
**alexandria — which ai, then four actions for that app**

## which ai do you use most?

1. add the Shortcut — capture thoughts wherever you are.
2. leave an email if you want a recovery copy.
3. copy the alexandria instructions
   paste in settings → … (that app's official account-instructions setting)
   after copy: copied, followed by the same `paste in` path.
4. copy the setup — paste in a normal chat.
...
Browser Back returns to the ai picker. There is no on-page different-ai control.
