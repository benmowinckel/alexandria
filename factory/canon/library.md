...
PLM inference is a separate optional stand module, not a Library requirement. Every Author who chooses hosted relay connects a sidecar that uses a model account and token they control; the company token is never a fallback. They may choose any model or conforming endpoint. Without that connection, inference is offline while the profile and files continue to work. Full hosted-relay mechanics: `plm.md` and `twin.md`. A static public mirror and an own-host model are separate independent choices; neither needs the relay. Existing websites follow `connector.md`.
...
The ai may browse the Library only when the Author directly asks, or when the chosen client's `permissions/people-context` contains exactly `on` (`~/alexandria/system/` for a private loop; `~/.config/alexandria/connector/` for the standalone client). That marker is narrow standing consent for one behavior during ordinary work: when a specifically named person materially affects the task, check whether that person is an Author and use what this account can already read.

Use the signed local `scripts/person-context.mjs` from the chosen client (`~/.local/share/alexandria` or `~/.local/share/alexandria-connector`); never improvise a search from the private prompt. The standalone reader needs `ALEX_CONNECTOR_DIR="$HOME/.config/alexandria/connector"` on member commands so it reads its own key and permission; it must not silently fall back to a loop client. Exact commands are in the installed `canon/connector.md`. `directory` reads one bounded page of at most 25 candidates and sends no name or private text, so identity matching happens locally. Its response carries `next_cursor` and `directory_complete`; only a deliberately requested next page sends that exact reader-bound cursor. An empty or short incomplete page does not prove someone is absent. Never automatically download the whole roster. Use a known verified handle directly with `person`; otherwise fetch only the pages the current task needs. Exact paging commands and the one-hour cursor expiry are in `connector.md`. Continue only on one confident identity match; ask one plain question when two people plausibly match, and skip incidental names whose context would not change the answer. `person` first resolves the matched Author’s verified website registration. When present, it reads that exact public manifest and same-origin public shadows directly, with no account key or cookies forwarded to the website; unavailable registered content fails explicitly rather than silently substituting an older hosted copy. Without a verified site, it loads the existing hosted profile, accessible shadows and artifact list. `file` remains a separate exact authenticated read for hosted protected material. Public own-site data conveys no extra access; members, paid and exact invite grants still pass through the current server authority, and a code-bound grant needs no repeated code. The caller's ai performs the personalisation locally. It never queries the other person's PLM and never sends the current prompt, private files, inferred relationship, or local context to Alexandria or to the other Author.
