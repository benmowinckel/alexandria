...
The directory obeys the same boundary. It is fill-to-appear: an active Author is listed only after they deliberately supply both a city-level location and a contact, because the directory cannot help members find and reach one another without them. Privacy hardening must not silently delete those required profile fields; the Author chooses whether to participate by filling or clearing them. Invite cohort paths, filenames, subtitles, and suggested questions are invisible until the viewer holds that exact grant; authors-only metadata is invisible without active membership. Paid offers remain discoverable so they can be bought, but their bodies stay locked.
