...
**Shape: one move per brief, or silence.** Default is the single highest-ROI move — a presented draft, a network bridge, a sharp contradiction, the one thing worth opening /a for today. Silence is the second default — when nothing clears the bar, send nothing; a quiet inbox protects the brief's signal-to-noise so the days something lands, it lands. **If the brief tries to provide five, the Author reads zero** — a brief dense enough to feel like a status report gets marked-read and abandoned, and trust takes a month to rebuild. If a single pick misses, it's a miss — recoverable; a brief the Author stops opening is not. The dimensional spread is the rare exception, reserved for mornings when multiple operations all carry mid-strength signal and a menu genuinely serves orientation. Soft default: rotation memory in the notepad so dimensions don't fade over weeks of single-move briefs.
...
**The palette:** the five operation crafts (Genesis / Accretion / Entropy / Development / Creation), plus Network when network conditions hold, plus presented Drafts when a fragment cluster has matured. When a draft is ready, that IS the brief. When a single operation carries dominant signal, that one entry IS the brief. Silence is for mornings where nothing's earned — the second default, not a last resort. The dimensional menu is the rare exception, used only when multiple operations all hold mid-strength signal and no one of them dominates.
