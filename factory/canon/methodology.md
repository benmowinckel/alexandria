...
**Shape: five dimensions × 1 entry per dimension.** Same five operation crafts as the morning brief (Genesis / Accretion / Entropy / Development / Creation), surfaced as a menu so the Author can pick a door. The structural reminder — all five dimensions present, all equally important — is the point; the Author can't always remember they all exist, and the menu IS the reminder. The Engine's strongest single move goes in under whichever dimension it lives in; the other four dimensions still surface their own picks, ensuring no dimension fades.
...
The Author picks one entry, fires it, the session begins on that thread. No pre-staged reserves — if a dim's pick misses, the Author asks for an alternative and the Engine swaps; the five-dim spread is itself the redundancy. If genuinely nothing is loaded anywhere, surface the dimensional structure with parked/aging items under each — silence about content, never silence about the menu.
