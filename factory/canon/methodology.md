...
**Session opener.** After session-start checks land in their files (intake processed, shadow drafts staged, drift notes captured), the first response in chat is the cognitive opener — the **dimensional menu**. Plumbing reports, queue summaries, and "fifth /a today" framing stay in files, not in chat.

**Shape: five dimensions × 1 main + 2 reserve options per dimension.** Same five operation crafts as the morning brief (Genesis / Accretion / Entropy / Development / Creation), surfaced as a menu so the Author can pick a door or a combo. The structural reminder — all five dimensions present, all equally important — is the point; the Author can't always remember they all exist, and the menu IS the reminder. The Engine's strongest single move goes in as the main under whichever dimension it lives in; the other four dimensions still surface their own mains, ensuring no dimension fades.

Per-dim entry quality matches the morning brief: each entry carries enough context to entice — a hook, not a label. Specific topic + why it's loaded right now + the invitation. "AI agency incomplete sketch" is a label and fails; "AI agency — your spawn-vs-supervise cut still hasn't landed; let's pressure-test it against a concrete case" is a hook and lands. Sourcing per dim mirrors the brief (Genesis = silent/news/incomplete; Accretion = vault fragments + shortcut sub-section; Entropy = drift + dark domains; Development = under-developed positions + edges; Creation = coalescing topics ready for articulation). If under a dimension nothing fresh exists, surface parked items rather than dropping the dim — five always appear.

The Author picks one entry (main or reserve from any dim), fires it, the session begins on that thread. If genuinely nothing is loaded anywhere, surface the dimensional structure with parked/aging items under each — silence about content, never silence about the menu.
