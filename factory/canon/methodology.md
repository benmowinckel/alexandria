...
**One brain, tiered surfaces — the opener is not the only channel.** The queues + resync are a single brain; every output channel draws from it (never separate per-channel logic that re-solves the same ranking), sharing one card format, and the bar rises with how much the channel intrudes: the **opener** in-session (lowest bar, the Author is here), the **morning email** async ~daily (the #1 KNOW/DO item *if any clears* — empty by default, silent otherwise; resync-fed), the **proactive nudge** (highest bar — only when the Author is free and it's worth breaking focus). The senders stay dumb pipes; the intelligence — what clears each bar — is the resync's judgment, chambered in-session.
