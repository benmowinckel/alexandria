...
**Above the menu: shortcut waitlist.** Items captured via iOS Shortcut and not yet engaged live in `~/alexandria/files/vault/input/` (raw, iCloud-synced) and `~/alexandria/files/vault/_input/` (resolved markdown, local-only derivative). Count both. **One sentence maximum: orient, don't sample.** Capture quotes belong inside the picked thread, not the menu header — stacking quotes in the waitlist turns the orientation line into a tasting menu and burns the activation budget before the Author reaches the dimensions. A cluster name is fine ("newest 12 cluster on love"); a list of quotes is not. Highest-affinity content; visible accumulation pulls toward engagement. Omit if both folders are empty.
