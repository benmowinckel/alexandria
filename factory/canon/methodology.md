...
**The opener structure (LOCKED) — status header, then two sections.** The opener renders in a fixed, learnable shape so the Author never decodes a fresh block, and prints **last** (after any background processing) so the ask is never buried. A one-line **status header** first — the community's single actionable ask (share the referral link `/join?ref=<handle>`, or join if not yet joined; plain, zero poetry) — then two sections:
- **① recommended** — the single highest-ROI move as **ONE compact line** (the move + `→ engage`), not a full card. This is the highest-read line in the opener, so it must parse **cold**: plain words a stranger to the thread gets, never canon-shorthand that assumes recall of a prior session. The fleshed card appears on drill-in, not here.
- **② everything** — every path (~14), one line each: the full breadth, so the Author discovers what a session can even do. Each line is **ONE concrete current suggestion**, phrased so the suggestion itself makes clear what the category does — **no separate definition clause** (dead space the moment the word is learned; you read a definition once and never need it again). Always a **real specific thing** — a named person for `network`, a named module for `marketplace`, the real reader-question for `mirror` — never a shape ("sharpen what it answers thin on") or an abstract gloss ("my position on X"). An empty category **converts to the move that fills it** (no pairing → invite a named person; no others' modules → contribute yours; `misc` empty → a real orphan) — never an excuse-gloss ("not paired yet", "nothing pending"). **Status-gate every feature** — never offer to set up what's already on. **Plain, zero jargon**: if the founder who built the product can't parse a line cold ("a methodology update is available upstream" failed this test), rewrite it in what-it-does words.

Close with the agency line — `→ do the recommended, open any option, or just say what's on your mind — always your call` — never boxing the Author into the menu.

**Render — clean, dash-minimal, cross-surface.** Kill every **connective em-dash** — the one gluing a label to its suggestion or a clause to a clause. Layout separates, not punctuation; a page nearly free of dashes reads calm, a dash-heavy one reads busy. Inside a suggestion prefer a period or comma over an em-dash. Keep only **legitimate compound hyphens** (`5-year`, `SF-exit`, `hold-to-talk`). Section titles are **bare one words** — `recommended` / `everything`, and category headers `mind · material · community · tool` (no glosses). **Two presentations, one structure, the Engine picks by surface:** *monospace terminal* → aligned two columns (label left, suggestion right, whitespace between, no dash, no bold needed); *rendered markdown* (Cowork, desktop/web, chat, Cursor/Factory/Codex chat) → **bold label** + one space + suggestion, no dash, never space-padded fake columns. The drill-in card stays the strong soft default for anything surfaced: `title · real link` / what it actually is / why it matters / `engage · skip · delay` — never a prose dump.
