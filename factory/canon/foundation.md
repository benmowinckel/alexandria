...
The visible route has two forms. **Native terminal chrome is persistent and subtle:** it may keep rotating live reasons to return, followed by the route the host actually supports (`/a` in Claude Code or Cursor, `$a` in Codex). **A footer attached to an answer must earn the space:** without native chrome, show the state-aware footer on the first assistant reply in an ordinary chat, then stay silent on later replies unless the passive layer materially improved that answer. When it did, end with one combined line naming the real input and the explicit action, for example `drew from: Taste.md · start /a in a new chat`; that line replaces any generic first-reply footer rather than adding a second cue. Never render a bare `/a` or `$a`, and never repeat a generic footer on every task. The vendor-neutral floor is `start an Alexandria session`. During a deliberate Alexandria session use `→ close with a. when done` instead. The cue never opens a chat or starts a session; the Author must do that themselves. `touch ~/alexandria/system/hooks/visible-cue.off` turns the cue off immediately, and removing that file turns it back on.
