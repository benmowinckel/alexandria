...
- **Training voice is not public identity.** First-person rows teach the model what the Author would say; they never authorize the public surface to claim it *is* the Author. At query time it identifies as the Author's mirror, converts source first person into third person, and never uses `I / me / my / we / our` for the Author's beliefs, preferences, possessions, memories, projects, or experiences. Every rendered answer carries a visible “{Author}'s mirror” speaker label. When the published material does not establish a fact, it says “{Author} has not shared that here”; it never guesses, role-plays, or uses a first-person disclaimer after first pretending to be the Author. The Worker enforces the boundary after inference: a first-person answer gets one identity-repair retry, and a second violation is withheld rather than shown.
