...
`~/alexandria/files/library/` has four default permission sub-folders: `public/`, `authors/`, `invite/`, `paid/`. Those permission meanings are the stable contract; the literal folders are only the standard adapter. Canonical hand-authored works may remain anywhere in the Author's own structure. The simplest mapping is a file symlink from the exact Library permission folder to that source, with the approval sidecar beside the symlink. Named cohort folders may nest below a permission, such as `invite/friends/`; every scope is exact and nothing inherits. Placement is not sufficient consent. For a direct publication request, the Author must see the exact current target bytes and exact scope; after a separate yes, its adjacent `<filename>.approved` records the SHA-256 of those bytes plus that scope. A later edit or move invalidates approval and stays local until approved again. Drafts and every file outside the mapped Library outbox never ship.
...
The shared Library accepts at most 250 files, 25MB per file, and 250MB total per account. These are hosting boundaries, not creative structure: larger media and custom applications stay on a host the Author chooses and enter as links. Read the live capability response before publishing because ceilings may change with real usage.
