...
assert.match(initial, /codex, claude, cursor/);
assert.doesNotMatch(initial, /eg |claude code/);
...
assert.match(computerBody, /add the shortcut — save thoughts on your phone/);
assert.doesNotMatch(computerBody, /optional/i);
...
assert.match(computerBody, /— get the setup text and help/);
...
assert.equal(await page.locator('.act-num').count(), 3);
assert.match(cloudBody, /add the shortcut — save thoughts on your phone/);
assert.doesNotMatch(cloudBody, /optional/i);
assert.equal(await page.locator('.shortcut-add').evaluate((node) => node.scrollWidth <= node.clientWidth), true);
assert.match(cloudBody, /— get the setup text and help/);
