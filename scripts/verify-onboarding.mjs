...
assert.match(CHAT_INSTRUCTION, /ordinary conversation, including voice, end only your first reply/);
...
assert.doesNotMatch(CHAT_INSTRUCTION, /attached files|Before normal saves/);
...
assert.match(CHAT_SETUP_PROMPT, /without opening personal content or changing anything/);
...
assert.match(CHAT_SETUP_PROMPT, /what is ready and exactly where it persists/);
...
await page.goto(`${base}/chat`, { waitUntil: 'networkidle' });
await page.getByRole('button', { name: 'another ai' }).click();
await page.waitForURL(/#other$/);
assert.match(await page.locator('body').innerText(), /its persistent instructions, if available/);
await page.getByRole('button', { name: 'copy the instructions' }).click();
assert.equal(await clipboard(), CHAT_INSTRUCTION);
await page.getByRole('button', { name: 'copy the setup' }).click();
assert.equal(await clipboard(), CHAT_SETUP_PROMPT);
await assertFits('.instruction-copy');
