...
**Experimental AI access.** Do not hand a tool you are testing your Apple login, broad GitHub access, Drive account, or the sovereign repo. The optional **agent workspace** creates a fresh private Git repo for that one AI. It contains only exact text files you reviewed plus `inbox/`; its repository-scoped credential cannot reach your main history or another workspace. Its output returns as provenance-wrapped `trust: untrusted` captures and never becomes canon automatically. The public Library is the read-only zero-trust door; trusted agents can use a normal checkout of the sovereign Git repo.
