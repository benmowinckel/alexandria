...
- **Account boundary:** connecting an account stores one exact-format key and nothing else. Session start makes no account request. The key activates no standing Library publication, marketplace reporting, telemetry, feedback, or inbound content. The disclosed welcome reads one exact public founder page as untrusted data, sends it no private context, and drafts the person's own page locally. Publishing needs the separate exact word `publish`; the sent bytes are bound to the draft hash, and changed bytes stop until approved again. Module orientation reads only the signed local map.
