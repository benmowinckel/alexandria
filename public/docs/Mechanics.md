...
- **What runs:** plain bash, Python, and markdown. Your editable files live at `~/alexandria/`; automatic hook programs and their verification markers live separately at `~/.local/share/alexandria/`, outside the writable root granted to the AI. The hooks point the AI to your files and save transcripts when the host exposes them (Cursor builds its own local transcript). One small `/a` cue appears after completed tasks; it never opens a chat, and one local file turns it off immediately. No new app, daemon, launchd/cron job, shell-rc edit, or root process.
- **What the install does NOT do:** no cloud connection, push to any remote, repo creation, key upload, or scheduled job. iCloud capture, Google Drive, backups (to your **own** GitHub/iCloud), Library publication, marketplace signal, and network reading are separate opt-ins — each needs its own explicit yes after install (`~/alexandria/system/.optional` documents every one: what it touches, what leaves the machine, its off switch).
...
- **What our server holds if you create an account:** your email, GitHub user ID, hashed API key, a 60-day event log of which endpoints you hit, and any files you explicitly publish to the Library. The phone's one-time setup email is delivered without creating an account, waitlist entry, reminder sequence, or install record.
...
Start at [`/start`](https://alexandria-library.com/start). Its message names `github.com/benmowinckel/alexandria`, tells your agent to trust none of our files or setup instructions, and leaves the security process to that agent. Nothing changes until the agent explains its findings and you say `start`.
...
| `system/canon/` | Signed local references, cached once and never auto-written afterwards. **Core:** `foundation.md`, the complete local loop. **Replaceable defaults:** `axioms.md`, `methodology.md`, `editor.md`, `mercury.md`, `publisher.md`. **Dormant until directly chosen:** Library/filter, marketplace, and PLM/twin. `MODULES.md` is the exact activation map; cloud/network choices live in `.optional`. A file being available here does not activate its external feature. Signed updates are notices only; you pull one or ignore it. |
...
**`~/.claude/skills/{a,alexandria}/SKILL.md`** — the `/a` skill (and its `/alexandria` alias). **`~/.claude/skills/a./SKILL.md`** — the `/a.` close skill: ends an /a session by capturing everything to your files, then asking what shifted — you say it, in your words; it files them. Plain markdown, all of them. `cat` them. (Cursor gets the same three under `~/.cursor/skills/`.) If `a` or `a.` already belongs to another tool, setup leaves it untouched and uses the namespaced Alexandria alias instead.
...
The hooks activate only after the full local core passes its functional checks. If setup or an update stops halfway, the partial files remain available to inspect and repair, but every Alexandria hook stays off until a verified rerun succeeds.
...
| `PUT api.alexandria-library.com/file/<name>` | Only with Library permission and a matching `<filename>.approved` content hash | API key, that exact approved file + visibility tier; no adjacent metadata or other private path | 200/4xx |
