...
**Root changes.** `root` is a mark inside the living Constitution, not a second identity file or a claim that an untouched self exists. It creates the operational incompressible core: adding the mark, changing or deleting the position, and removing the mark all run the same gate. The proposing AI records before→after, both cases at full strength, its reason, and its own model/provider/harness identity where available. A reviewer from a different provider and independently trained base-model family must mark the review complete; another session, alias, version, or reasoning mode of the proposer does not count. The Author then signs the substantive change in their own words. Until all three gates close, the old state remains current. Each model's account of its influence is self-reported and Author-correctable; it is provenance, not authentication.

The pending packet lives in marginalia—the shared working layer every supported AI reads—so one model can hand the same transition to another through files alone. At session start and before a Constitution commit, a conforming Engine compares the root set and root passages against Git. An unauthorized addition, rewrite, deletion, or unmarking is restored from the last commit and left pending. This is model-enforced semantic review over a file-and-Git substrate, not a cryptographic write lock: an Engine that ignores the instructions can bypass it, but the committed history makes the bypass visible and recoverable to the next conforming Engine. A hard-coded text matcher would be stricter about syntax but blind to the same position being smuggled through different words.

**The provenance stack.** The transcript or vault preserves what was actually said; the delta packet preserves the before→after, arguments, influences, model identities, and human correction; Git preserves byte history and key continuity. None alone proves inner authorship. Together they make the causal record inspectable without pretending influence can be eliminated.
