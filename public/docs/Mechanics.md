...
- **What runs:** plain bash, Python, and markdown. Your editable files live at `~/alexandria/`; automatic hook programs and their verification markers live separately at `~/.local/share/alexandria/`, outside the writable root granted to the AI. The core is one local loop: ordinary sessions use your approved mirror and save transcripts when the host exposes a regular user-owned file under a supported host root; persistent native terminal chrome, one direct daily host notice where the hook API guarantees it, or a model-mediated fallback links into an active session; the host's installed Alexandria skill develops what accumulated and `a.` preserves what changed. Codex uses its unchanged trusted SessionStart command to show the consent question when the first startup, resume, or clear turn of each local day starts after onboarding; compaction and subagents do not claim it, and private Author context stays hidden in the hook's separate context field. The notice appears after the prompt is submitted but before the model answers. SessionStart receives event metadata rather than the prompt content, so it cannot know whether the turn is voice, review, setup, or `$a`. Hosts without a direct surface keep the offer on the first ordinary text reply and can apply those semantic exclusions. The offer opens nothing before you agree; a capable host then opens a new chat and invokes its native skill, while an incapable host tells you the exact gesture to use. Later replies mention Alexandria only when it materially helps that exchange. There is no Stop loop, response rewrite, forced second turn, new hook definition, or extra trust prompt. In slash-command hosts the start is a slash skill; in Codex it is a dollar-sign skill. The portable floor is the request `start an Alexandria session`; the plain word `alexandria` is not a universal skill trigger. No new app, daemon, launchd/cron job, shell-rc edit, or root process.
...
On the later route, the receiving AI does not inspect the project or begin setup. It creates one lasting reminder only through a reminder, task, or other feature it can verify will reach the person's computer. It asks one short timing question if needed, reports exactly where the reminder was saved, and says plainly when it cannot make the reminder persist.
...
**Experimental AI access.** Do not hand a tool you are testing your Apple login, broad GitHub access, Drive account, or the sovereign repo. To connect one, open the computer AI that already has Alexandria and type `Airlock this new AI: <name or URL>`. It creates one fresh private repo for that AI and tells you the exact repo to select in the new AI's GitHub permission screen; you never create or manage the repo yourself, and repos are never reused between untrusted AIs. **Airlock** is a disposable projection of approved context plus an untrusted inbox. By default it mirrors your already-public Library context and keeps it current. A new private file crosses only after exact approval and stays frozen until reapproved. The app returns work through `inbox/` files or `airlock-capture` GitHub issues; GitHub is replaceable transport, not ground truth. Every return becomes a `trust: untrusted` capture with no automatic authority to change files, run tools, or become canon. The public Library is the read-only door; trusted agents can use a normal checkout of the sovereign Git repo.
