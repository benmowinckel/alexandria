...
The chat path asks whether you use ChatGPT, Claude, Gemini or another AI, then gives two actions: copy the additive Alexandria instruction into that host's instruction field; paste one short setup request into a normal chat. The receiving chat verifies the instruction, identifies the writable storage or memory it can actually use, and asks direct permission before opening the named personal sources or changing anything. After permission, it builds and reads back the first record and begins from one specific live thread rather than generic onboarding. A failed write or read is reported as a failure, never disguised as success. The final orientation explains normal use, the host's actual start gesture, `a.` to close, confirmation before saving lasting changes about the user, and how to change or remove the instruction and record.
