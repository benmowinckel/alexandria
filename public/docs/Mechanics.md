...
- **Account boundary:** connecting an account stores and validates its key. Session start then makes one small authenticated status read: API key + client-version hash out; live account state + the current static module-map version back. It sends no Author file or private content and activates no Library publication, marketplace reporting, network reading, telemetry, or feedback. Each data-carrying connection has its own consent record tied to the exact outbound file or list. Changed bytes stop until approved again. The private ai never drafts or proposes Alexandria requests, contributions, pricing, or feedback. For a verified active member, `/a` may show their fixed invite link and a one-time or changed-version orientation from the signed local module map; neither may use private material or activate anything.
...
If you later want another computer connected, ask the AI that already runs your loop. It creates the exact text to paste into the AI on the other computer. That AI independently checks the signed connector, explains the two local files and the small network exchange, and waits for the exact word `connect`. The short-lived connection code is consumed once. A persistent API key is created only during that exchange, never placed in browser HTML, email, or the human paste. Connection does not rerun setup or enable any optional capability. The welcome page and welcome email never ask the person to make this decision.
...
| `system/modules.json` | The signed machine-readable map of core, removable methods, available additions, and separately approved connections. The authenticated handshake carries its version only so your ai can notice a real packaging change; local files and permission markers remain the authority for what is actually on. |
...
2. **Awareness (one-time, additive).** Working Alexandria hooks are primary. Setup writes compact Alexandria instructions to `~/alexandria/system/.account-instructions.md`; after the first private result works, the agent helps paste them below the current instructions in every other AI the person says they use, one at a time. The rules stay in instruction settings, never account memory. For Cowork and similar folder-only tools, the Alexandria instructions open `_start` at each task because no Alexandria hooks run there.
...
**They start off.** To enable signed, notify-only checks after a separate yes: `touch ~/alexandria/system/hooks/auto-update`. To stop them: `rm ~/alexandria/system/hooks/auto-update`. Removing the marker keeps every session on the pinned local copy, and re-running setup does not turn checks back on. A connected account still performs its small status read; that read can report a newer module-map version but cannot fetch, apply, or execute an update. Separately approved Library, marketplace, backup, saved-link resolution, or network activity stops by removing its file under `~/alexandria/system/permissions/`.
...
| `POST api.alexandria-library.com/account/connect/exchange` | Once, only after the exact word `connect` | short-lived one-use code, installed public client version, and the current connected login/key when one already works | one new account key, or confirmation that the existing key remains valid |
| `GET api.alexandria-library.com/alexandria` | Once when you separately say `connect`, then at session start while that account remains connected | API key (Bearer) + client-version hash | live account state + current static module-map version; no Author files or private content |
...
| Account connection code — SHA-256 hash, account pointer, and expiry (24 hours or first use) | D1 | One explicit connection exchange without exposing a persistent key in browser HTML or email |
