...
- **What runs:** plain bash, Python, and markdown. Your editable files live at `~/alexandria/`; automatic hook programs and their verification markers live separately at `~/.local/share/alexandria/`, outside the writable root granted to the AI. The core is one local loop: ordinary sessions use your approved mirror and save transcripts when the host exposes them; persistent native terminal chrome, or one cue on the first reply in a new text chat, links into an active session; `/a` develops what accumulated and `a.` preserves what changed. Voice omits the generic cue, and later chat replies mention Alexandria only when it helps that exchange. The cue never opens a chat, and one local file turns it off immediately. No new app, daemon, launchd/cron job, shell-rc edit, or root process.
...
Start at [`/start`](https://alexandria-library.com/start). Choose agent or chat. If you choose agent, say whether your computer is nearby; the page then gives your current AI the matching computer or phone handoff. Email is optional. Every message names `github.com/benmowinckel/alexandria`, tells your AI to trust none of our files or setup instructions, and leaves the security process to that AI. Nothing changes on your computer until it explains its findings and you say `start`.
