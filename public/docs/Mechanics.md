...
Start at [`/start`](https://alexandria-library.com/start). Choose agent or chat; that line copies the right request and becomes an exact paste instruction while the other choice fades without moving anything on the page. Paste it into the AI you chose. The agent request asks the receiving AI to check its real capabilities instead of asking which device or app you use. If it can run commands and read and write your computer files, it independently inspects `github.com/benmowinckel/alexandria`; nothing changes until it explains its findings and you say `start`. Otherwise it creates and verifies one reminder or temporary instruction that brings setup back when your computer is reachable, without pretending the local product is installed.

The chat path gives one universal paste. The receiving chat identifies only settings and capabilities it can verify, guides you to append the Alexandria instruction without deleting anything already there, connects its own native Drive route when available, asks direct permission for the exact personal sources it can use, builds and reads back the first record, and runs a miniature personal loop. Only after that works does it explain the optional computer-agent version. A fresh chat using that host's native Alexandria skill is the final proof; where no native skill exists, the floor is the request `start an Alexandria session`. It must read the record and begin from one specific live thread rather than generic onboarding.
...
After joining, the signed-in welcome page links to one exact handoff to paste into the AI on your computer. The browser session proves only which account is asking and mints a short-lived, one-use connection code; it never receives the persistent machine key. Your AI independently checks the signed connector, explains the two local files and the small network exchange, and waits for the exact word `connect`. A persistent API key is created only during that exchange, never placed in browser HTML, email, or the human paste. Connection does not rerun setup or enable any optional capability. If you later want another computer connected, ask the AI that already runs your loop; it creates the same bounded handoff for that computer.
...
2. **Awareness (one-time, additive).** Working Alexandria hooks are primary. Setup writes compact Alexandria instructions to `~/alexandria/system/.account-instructions.md`; after membership is connected, the agent helps paste them below the current instructions in the one other ai app the person uses most. Later active sessions can add another when it is relevant. For Cowork and similar folder-only tools, the Alexandria instructions open `_start` at each task because no Alexandria hooks run there.
