...
Start at [`/start`](https://alexandria-library.com/start). Choose agent or chat, then make one paste. The agent request asks the receiving AI to check its real capabilities instead of asking which device or app you use. If it can run commands and read and write your computer files, it independently inspects `github.com/benmowinckel/alexandria`; nothing changes until it explains its findings and you say `start`. Otherwise it creates and verifies one reminder or temporary instruction that brings setup back when your computer is reachable, without pretending the local product is installed.
