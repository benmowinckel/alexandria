...
- **Account boundary:** connecting an account stores one exact-format key and nothing else. Session start makes no account request. The key activates no Library publication, marketplace reporting, telemetry, feedback, or inbound content. Each data-carrying action has its own consent record tied to the exact outbound bytes; changed bytes stop until approved again. The private ai never drafts or proposes Alexandria requests, contributions, pricing, referrals, or feedback. Module orientation reads only the signed local map.
...
If you join the optional community, the welcome page gives you one opaque connection code to paste into your computer agent. Your agent explains the narrow change and waits for the exact word `connect`. The signed connector then exchanges that one-use, one-hour code for an exact-format account key and stores only that key at `~/alexandria/system/.api_key`. It never prints server text, reads account status, reruns setup, or enables another capability. The welcome email keeps the normal private-session route and carries no connection secret.
