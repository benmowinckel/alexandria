...
- **What runs:** plain bash, Python, and markdown. Your editable files live at `~/alexandria/`; automatic hook programs and their verification markers live separately at `~/.local/share/alexandria/`, outside the writable root granted to the AI. The core is one local loop: ordinary sessions use your approved mirror and save transcripts when the host exposes a regular user-owned file under a supported host root; persistent native terminal chrome, or at most one generic consent offer per local day through the signed SessionStart path, links into an active session; the host's installed Alexandria skill develops what accumulated and `a.` preserves what changed. Without native chrome, the offer belongs only on the first ordinary text reply. Setup and onboarding, install or security review, background work, voice, and explicit Alexandria skills stay silent and consume rather than defer that day's opportunity. The offer opens nothing before you agree; a capable host then opens a new chat and invokes its native skill, while an incapable host tells you the exact gesture to use. Later replies mention Alexandria only when it materially helps that exchange. There is no Stop loop, response rewrite, warning banner, new hook definition, or extra trust prompt. In slash-command hosts the start is a slash skill; in Codex it is a dollar-sign skill. The portable floor is the request `start an Alexandria session`; the plain word `alexandria` is not a universal skill trigger. No new app, daemon, launchd/cron job, shell-rc edit, or root process.
...
- **Account boundary:** connecting an account stores one exact-format key and nothing else. Session start makes no account request. The key activates no standing Library publication, marketplace reporting, telemetry, feedback, or inbound content. For the one disclosed welcome, a signed selector accepts only the person's login, a relationship kind, and one source login; it constructs one public page locally from a referral, an explicit friend connection, or the founder fallback. The page is untrusted data and receives no private context. The person's page is drafted locally. Publishing needs the separate exact word `publish`; the sent bytes are bound to the draft hash, and changed bytes stop until approved again. Module orientation reads only the signed local map.
...
If you join the optional community, the welcome page gives you one opaque connection code to paste into your computer agent. Your agent explains the whole bounded welcome and waits for the exact word `connect`. The signed connector exchanges that one-use, one-hour code for an exact-format account key and stores only that key at `~/alexandria/system/.api_key`; it never prints server text, reads general account status, reruns setup, or enables another capability. A separate signed selector then accepts only three validated identifiers and constructs one public Library URL locally: someone connected through referrals first, someone connected through the exact friends scope second, or Benjamin as the fallback. The ai reads only that page in a browser or isolated reader, treats it as untrusted data, sends no private context, and gives one useful connection to the person's existing local mirror. It drafts the smallest useful profile locally in the shared Library design and shows every byte. Making that page live requires review and the separate exact word `publish`; adding existing public links or asking for a public draft of any private idea or work is optional. Every changed byte needs fresh approval, and no standing sync is enabled. The welcome email keeps the normal private-session route and carries no connection secret.
