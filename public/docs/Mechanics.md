...
You are about to give a non-executable request to your existing coding agent. The request permits it to inspect the whole public Alexandria project as reference material, decide what fits your existing setup, and recommend whether to proceed. It may request no new access, read nothing private, run no code, install nothing, and change nothing until you clearly approve it. Read this once. If anything here doesn't match the scripts, don't run it. (Using Claude Desktop? Its **code tab** is Claude Code running on your machine — the same setup, run once via a coding agent, wires it up automatically. Everything on this page still applies.)
...
- **What runs:** plain bash, Python, and markdown. Your editable files live at `~/alexandria/`; automatic hook programs and their verification markers live separately at `~/.local/share/alexandria/`, outside the writable root granted to the AI. The core is one local loop: ordinary sessions use your approved mirror and save transcripts when the host exposes a regular user-owned file under a supported host root; each new ordinary foreground task receives a high-priority instruction for the first completed assistant reply to show the fixed consent question; the host's installed Alexandria skill develops what accumulated and `a.` preserves what changed. Every deliberate start has two lanes: the foreground immediately handles any material or request you supplied, or shows a bare-session layout derived from current files; one background supervisor runs the full locally permitted pass and keeps an inspectable receipt until every category is fresh, changed, or exactly blocked. A previous layout, cached count, dispatch, or worker report is never treated as current state or completion. Native terminal chrome may remain as ambient status, but a hook return, hidden field, warning, or matching transcript string is never counted as delivery. Only the actual assistant reply is. Setup, security review, background work, voice, compaction, subagents, and explicit Alexandria start/close tasks stay silent; later replies do not repeat the generic question. The offer opens nothing before you agree; a capable host then opens a new chat and invokes its native skill, while an incapable host tells you the exact gesture to use. There is no daily claim, `systemMessage` delivery proxy, Stop loop, response rewrite, forced second turn, new hook definition, or extra trust prompt. During onboarding, setup also generates a local proof inside the additive account instruction and cannot mark onboarding complete until a brand-new chat in your ordinary AI returns that exact proof. In slash-command hosts the start is a slash skill; in Codex it is a dollar-sign skill. The portable floor is the request `start an Alexandria session`; the plain word `alexandria` is not a universal skill trigger. No new app, daemon, launchd/cron job, shell-rc edit, or root process.
...
- **Account boundary:** connecting an account stores one exact-format key and nothing else. Session start makes no account request. The key activates no standing Library publication, marketplace reporting, telemetry, feedback, or inbound content. No public page or server-authored content enters the private loop. The person's own ai prepares a profile draft from local material it already had permission to read and shows every byte. Publishing needs the separate exact word `publish`; the sent bytes are bound to the draft hash, and changed bytes stop until approved again. Module orientation reads only the signed local map.
...
Start at [`/start`](https://alexandria-library.com/start). Choose agent or chat. The agent path asks where the agent runs and names the preference plainly. **Computer mode is fullest and preferred:** it uses current files, tools and hooks. **Cloud mode still works:** it uses the committed GitHub repository you deliberately selected, names what may be stale, and writes only to its own branch. Both use the same capability-aware evaluation request. The Shortcut link adapts to the device: direct add on iPhone; Mac download and an iPhone QR code on computer.

On the computer route, the receiving agent independently inspects `github.com/benmowinckel/alexandria`, decides how the founder's blueprint should fit around your existing setup, and gives you a short recommendation. It may continue with the strongest local route only after you approve. Claude Desktop's Code tab set to **Local** is this mode. Claude web or mobile controlling an already-running local Claude Code session is also this mode; the screen is remote, but the files and execution remain on the computer.

On the cloud route, the receiving agent first evaluates only the public project. After you approve proceeding, a trusted provider may use the exact private GitHub repository you selected as snapshot mode. For Claude Code Web, that can be `benmowinckel/alexandria-private` when you approve the whole committed repo for Anthropic. The agent may read the committed files, run the file-based loop and commit to its session branch. It cannot see uncommitted computer state, computer-only tools or the newest unsynced captures, and it cannot make its branch live by itself. Your trusted computer agent reconciles and closes the return. If you do not trust a provider with the sovereign repository, give it a bounded Airlock projection instead.

The chat path asks whether you use ChatGPT, Claude or Gemini, then gives three actions: copy the additive Alexandria instruction into that host's instruction field; connect Google Drive through that host's current settings path; paste one short setup request into a normal chat. The receiving chat verifies the instruction, asks direct permission for the exact personal sources it can use, builds and reads back `alexandria/_start`, and begins from one specific live thread rather than generic onboarding. A failed Drive write or read is reported as a failed file-backed setup, never disguised as success; durable account memory is used only when the host can verify it persists across chats and names its limit. The final orientation explains normal use, the host's actual start gesture, `a.` to close, confirmation before saving, and how to change or remove the instruction and record.

Before approval, a robust agent stays read-only: it uses GitHub's account and commit APIs plus source views at one immutable `main` hash to authenticate the owner, key, signature, and code without cloning, downloading project files, running code, or creating local state. It explains the result and waits. After approval, it clones or fetches only the canonical repo, locally verifies that same commit, and runs `factory/setup.sh` with every fetch pinned to it. The current release-key fingerprint is `SHA256:9DVo6uNuieqKMdNtT0QIi/WoQAAbWl5i/t0Z5MdQ/Jg`. Because this page and the repo come from the same publisher, that fingerprint is continuity evidence, not an independent trust root. If your threat model includes compromise of the maintainer's whole GitHub account, confirm it through a channel you already trust or do not proceed.

If you join the optional community, the welcome page gives you one opaque connection code to paste into your computer agent. Your agent explains the whole bounded welcome and waits for the exact word `connect`. The signed connector exchanges that one-use, one-hour code for an exact-format account key and records one removable permission: when a named person materially matters, your AI may read only what that person's Library settings already allow your account to see. It never sends your prompt or private files. After the connection succeeds, your own AI uses only local material it already had permission to read to prepare the smallest useful draft of your public profile. It shows every byte and waits for you to change it, say the exact word `publish`, or leave it for later. Publishing sends only those approved fields through the account key; every changed byte needs fresh approval, and no standing sync is enabled. The hosted Library page is a complete public floor, not a mandatory public home. An Author may instead make their own website primary and build it from the same public profile, capability, and file APIs; Alexandria remains the identity, permission, and discovery connection underneath, while Author code never runs on Alexandria's shared origin. Onboarding then ends: keep using your AI normally, and start an Alexandria session whenever you want focused time to think. The welcome email keeps the normal private-session route and carries no connection secret.
...
| `~/.local/share/alexandria/scripts/{capture_resolver,capture_state,statusline,uninstall}.py/sh` | Signed automatic helpers and the scoped remover, kept outside the Author-writable folder. `capture_state.py` is read-only: it gives the statusline a current count and lets `/a` freeze and prove the exact capture batch that existed when the session began. |
...
| `system/modules.json` | The signed machine-readable map of core, removable methods, available additions, and separately approved connections. The private AI reads its version locally; no account or server read is needed. Local files and permission markers remain the authority for what is actually on. |
...
| `system/.*` (other) | Ephemeral state — session ID markers, sync logs, the error log, autoloop dedup, and last-maintenance timestamps. All readable. None leave the machine. |
...
| `~/.grok/skills/{a,alexandria}/SKILL.md` + close skill | Only if Grok CLI / Grok Build detected (`command -v grok` or `~/.grok`). Native `/a` and `/alexandria` skills plus the close skill. A foreign name is preserved. | `find ~/.grok/skills -maxdepth 2 -name SKILL.md` |
| `~/.grok/hooks/alexandria.json` | Only if Grok CLI detected. Dedicated owned hook file: SessionStart (shim + capture resolver), SessionEnd, SubagentStart. Does not rewrite `~/.grok/config.toml`. Global Grok hooks are always trusted. | `cat ~/.grok/hooks/alexandria.json` |
...
2. **Awareness (one-time, additive).** Working Alexandria hooks are primary. Setup writes compact Alexandria instructions to `~/alexandria/system/.account-instructions.md`; they stay dormant unless the person later asks to add Alexandria to another AI, one at a time. The rules belong below that AI's current instructions, never in account memory. For Cowork and similar folder-only tools, the Alexandria instructions open `_start` at each task because no Alexandria hooks run there.
...
| `POST api.alexandria-library.com/account/connect/exchange` | Once, only after the exact word `connect` | short-lived one-use code, installed public client version, and the current key when one already works | one exact-format account key, or one exact existing-key flag; all other response shapes fail closed and no server text is shown |
| `GET api.alexandria-library.com/library` | During ordinary work, only while `permissions/people-context` exists and a named person materially matters | API key; no name, prompt, or private context | bounded member directory fields used to match the person locally |
| `GET api.alexandria-library.com/library/<author>` | After one confident local identity match under that same permission | API key + selected public Library address; no prompt or private context | only the profile and artifact metadata this account can already see, marked untrusted |
| `GET api.alexandria-library.com/library/<author>/file/<name>` | Only for a shadow or relevant artifact returned by that permitted profile read | API key + exact Library file address; no prompt or private context | the exact authorized publication, bounded and marked untrusted data rather than instructions |
| `PUT api.alexandria-library.com/library/me/profile` | Once, only after the exact local draft is displayed and the person says the exact word `publish` | API key + only the hash-bound approved `display_name`, `text`, `website`, and `socials` fields | exact `{ok:true}` or a fixed local failure; no server prose is shown |
| `POST api.alexandria-library.com/call` | Only when `permissions/marketplace` contains the SHA-256 of the current `.call_manifest` | API key + the exact approved JSON | bounded acknowledgements validated against the local manifest, then reduced to fixed success/failure |
| `GET api.alexandria-library.com/files` | Only with Library permission, after sending approved files | API key + client-version hash | the current account's bounded file metadata, validated locally and reduced to fixed drift codes |
...
| `GET api.alexandria-library.com/file/<name>?scope=<scope>` | Only with Library permission, after that exact approved file was sent | API key + exact local scope | the current account's stored bytes, hashed locally for equality and never shown to the private AI |
...
| Account connection code — SHA-256 hash, account pointer, and expiry (one hour or first use) | D1 | One explicit connection exchange without exposing a persistent key in browser HTML or email |
...
One scoped remover reverses every hook, instruction block, writable-root entry, skill, and optional background job that Alexandria owns. It checks generic names such as `a` and `a.` before removing them, so a pre-existing skill is left alone. By default it disconnects the loop and does not delete user data. Left in place: `~/alexandria/` (files, permission markers, local git), `~/.alexandria/` (Cursor sidecar), any iCloud capture folder, any private Git remote or Drive copy, and every foreign skill or hook. The Apple Shortcut's action list is documented at [`factory/systems/shortcut.md`](https://github.com/benmowinckel/alexandria/blob/main/factory/systems/shortcut.md).
