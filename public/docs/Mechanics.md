...
If you join after the local loop already works, account connection is a smaller, separate route. The welcome page and email copy a non-executable request pointing your AI to `factory/connect.md`; it independently checks the signed connector, explains the two local files and the small network exchange, and waits for the exact word `connect`. The short-lived connection code is consumed once. A persistent API key is created only during that exchange, never placed in browser HTML, email, or the human paste. Connection does not rerun setup or enable any optional capability. The welcome page never asks about another computer. Later, if you ask your connected ai to connect one, it creates the exact handoff text for the ai on that computer.
