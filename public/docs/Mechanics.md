...
| `GET alexandria-library.com/library/benmowinckel` | Once in the disclosed welcome, after the exact word `connect` and a successful key exchange | no private context or query; only the ordinary request metadata any website receives | one exact public page, treated as untrusted data in a browser or isolated reader and not retained as standing private context |
...
That is all. No telemetry pings, install reports, automatic feedback, error reporters, analytics SDKs, account-status reads, or automatic Library-page imports. Outside the exact disclosed welcome, public pages stay in the browser or a genuinely isolated reader and never enter standing private context; server text does not enter the private loop through connection or session start. The saved-link rows require both a separate permission file and a link you deliberately dropped into your own capture inbox; everything they fetch lands on your disk, not ours. You can confirm the full surface by `grep -E 'curl|wget|http' ~/.local/share/alexandria/.hooks_payload` and the same grep on `~/.local/share/alexandria/scripts/capture_resolver.py`. Capture extraction itself is local: `/a` checks the inbox against local completion proof and will not render its opener while unprocessed captures remain.
