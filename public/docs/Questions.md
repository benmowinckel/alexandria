...
Choose agent or chat. The agent path gives one request to the agent you already use. It says what that exact session can reach, reads the public project, recommends whether to proceed, and changes nothing until you approve. The chat path starts with one setup paste. That chat chooses the best record it can really write and read back, asks approval for personal sources and storage, verifies the result, then gives one personalized account-instructions block and the requested join link. A thinking session is optional afterwards.
