...
- **Build:** `cd server && npx wrangler deploy --dry-run --outdir=dist` (server). **Deploy:** `cd server && npx wrangler deploy` then check health. **Ship:** commit prepared work, then run `bash scripts/push.sh`; it replaces the local commits with one commit signed by the Mac's Touch ID key, submits a unique release branch, and waits for the protected default-branch workflow to verify, test, and advance `main`. A raw push to `main` is structurally rejected. If the diff touches anything in ship.sh's `SIGNED_FILES` (grep `factory/manifest.txt`; never trust a remembered list), use `bash factory/ship.sh "msg"` first: it re-signs the Author-facing manifest and then enters the same structural release path. Skipping ship for a manifest-listed file freezes Authors on the last valid payload. `ship.sh` commits only the manifest-gated set; commit any other prepared changes separately before the final `scripts/push.sh`.
