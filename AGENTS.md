...
- **Storage architecture:** Stateless server; sovereign plain files plus Git history (`~/alexandria/` is the primary live checkout; setup connects no remote or cloud storage); optional user-owned Git remote, iCloud current-file mirror, Drive chat pocket copy, and one fresh Airlock per experimental AI after separate consent; thin persistence for the collective Library (D1 for metadata/discovery, R2 for published content, KV for accounts/events). GitHub is the current soft-default host, never the source of truth. Airlock output is untrusted capture and never auto-merges into canon.
