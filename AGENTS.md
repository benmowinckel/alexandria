...
- **Build:** `cd server && npx wrangler deploy --dry-run --outdir=dist` (server). **Deploy:** `cd server && npx wrangler deploy` then check health. **Push:** `bash scripts/push.sh` (pushes + waits for CI + reports results). Always use `push.sh` instead of raw `git push` — **except** when the diff touches `factory/hooks/payload.sh` or any `factory/canon/*.md`: those are signature-gated, so use `bash factory/ship.sh "msg"` (re-signs `factory/manifest.txt`, then pushes). Skipping ship for canon/hook changes leaves the manifest out of sync, breaks payload signature verification for every Author, and silently freezes them on the cached payload. The inverse trap (bit 2026-07-17): `ship.sh` commits ONLY the manifest-gated set (canon + payload + the three gated skills + manifest; the harness /a skills claudecode/codex/droid are NOT gated — they ship via normal `push.sh`) — any other factory file in the same diff (`setup.sh`, `scripts/*`, templates) is silently left uncommitted in the working tree. After a ship, check `git status`: non-gated factory changes still need their own normal commit + `push.sh`.
