...
| Alexandria's server is compromised | It can reject an outward action, lie through a fixed success/failure status, misbind or disable an account key, or mishandle bytes the Author separately chose to publish. It cannot ask for more bytes or send prose, commands, public pages, diffs, or arbitrary JSON into the private AI through the account/session path. Public pages remain untrusted browser input when the Author explicitly opens them. |
