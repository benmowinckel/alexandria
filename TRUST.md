...
Alexandria is not an agent and not a service — it is a method your own ai runs: plain Author files at `~/alexandria/`, plus a small automatic runtime at `~/.local/share/alexandria/` that points your ai to those files and archives session transcripts locally. The AI is granted write access to the Author folder, not the runtime; a prompt that changes personal files cannot replace the next automatic hook or its verification marker. Your ai does the reading, writing, and learning; these files tell it how. Setup also adds one small visible `/a` cue after completed tasks; it never opens or starts a chat, and `touch ~/alexandria/system/hooks/visible-cue.off` turns it off immediately. Every session, the shim runs only the engine payload pinned in the protected runtime after that exact file has passed signature verification against a manifest signed by the maintainer's Touch ID-bound key. Nothing self-updates or checks for updates by default. Signed update notices are a separate opt-in, and applying an update is always your explicit action. This document explains the mechanism and what it does and does not defend against.
