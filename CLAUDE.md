...
- **The user's ai never sells Alexandria (2026-08-09; active-member opener carve-out 2026-08-13).** Alexandria-owned surfaces may carry the pitch. Installed prompts, onboarding, closes, nudges, and the user's private map never initiate pricing, membership, marketplace, feedback, or other company asks. One fixed active-session exception exists for Authors whose authoritative account state already says they are members: `/a` always opens in the exact order invite → cognitive recommended → everything. The invite body is the direct referral URL: rendered markdown uses an explicit full-address link and terminals use the plain URL. The model adds no mark, image, emoji, or hand-built preview; host-generated favicon or preview chrome is automatic and accepted. No custom share button or extra share page. It never uses private knowledge to sharpen that line. Unknown or inactive membership cannot produce an invite. This boundary and the exact carve-out are release-blocking.
