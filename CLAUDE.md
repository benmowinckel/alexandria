...
- **First touch is a read-first prompt, never a bare pipe (2026-07-28).** The thing a new user pastes into their agent is the *human's instruction to read before running*: download the script to a file, read that file, verify it against the named claims (TRUST.md; the hostile-audit prompt in `factory/redteam.md` is offered outright), then execute the same bytes that were read. Never ship a bare `curl | bash` as a first-touch surface: a cold, well-aligned agent refuses to blind-pipe from a marketing domain — correctly — and the whole trust apparatus is worthless if the first artifact carries no pointer to it (this killed real onboardings on 2026-07-22 and 2026-07-28). Re-run/sync contexts (an already-trusted install refreshing itself) may keep the raw one-liner. The claims inside the paste must stay exactly true — they are the first thing a skeptical agent verifies.
