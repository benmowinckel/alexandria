...
- **Build:** `cd server && npx wrangler deploy --dry-run --outdir=dist` (server). **Deploy:** `cd server && npx wrangler deploy` then check health. **Push:** `bash scripts/push.sh` (pushes + waits for CI + reports results). Always use `push.sh` instead of raw `git push` — **except** when the diff touches anything in ship.sh's `SIGNED_FILES` (the manifest-listed set — grep `factory/manifest.txt` for your file, never trust a remembered list: it includes `systems/`, `scripts/`, `tools/` too; a `systems/` push without ship went red on CI 2026-07-31): those are signature-gated, so use `bash factory/ship.sh "msg"` (re-signs `factory/manifest.txt`, then pushes). Skipping ship for canon/hook changes leaves the manifest out of sync, breaks payload signature verification for every Author, and silently freezes them on the cached payload. The inverse trap (bit 2026-07-17): `ship.sh` commits ONLY the manifest-gated set (everything in `SIGNED_FILES`, plus the manifest) — any other factory file in the same diff (`setup.sh`, `scripts/*`, templates) is silently left uncommitted in the working tree. After a ship, check `git status`: non-gated factory changes still need their own normal commit + `push.sh`.
...
- **First touch is the human's intent plus pointers — nothing executable, nothing claimed, nothing choreographed (v2, 2026-07-29; supersedes the v1 read-first script of 2026-07-28).** The thing a new user pastes into their agent contains: what the human wants (the act of pasting carries the intent — do NOT say so in the text: a vendor-drafted "this is me asking, not the website" was field-tested 2026-07-29 and an injection-paranoid agent called it "the single most suspicious feature… craft aimed at me"; never ventriloquize, not even to deny ventriloquizing), where the artifacts live (the script URL, TRUST.md, `factory/redteam.md`), the agent's own judgment as the explicit gate, a legitimate refusal path ("skipping it is fine"), and the human's one-line expectations (a couple of minutes; permission pop-ups are normal). It contains NO bash (a pipeline in the paste pattern-matches "random pasted command" before a word gets read), NO trust claims (vendor claims in the user's mouth are ventriloquism — claims live in the script header / TRUST.md / mechanics where they're accountable, and the agent verifies them there by its own plan), and NO step-by-step agent choreography (external text scripting the user's agent is the manipulation *shape*, however reasonable the steps — v1 made this mistake). History: bare `curl | bash` killed real onboardings on 2026-07-22 and 2026-07-28 — a cold, well-aligned agent refuses to blind-pipe from a marketing domain, correctly. Re-run/sync contexts (an already-trusted install refreshing itself) may keep the raw one-liner. Everything the paste does assert must stay exactly true — it is the first thing a skeptical agent checks.
...
Two runnable dev services (Node 22, npm; deps refreshed by the startup update script — `npm ci` at root + `npm --prefix server ci`). Commands live in `package.json` / `server/package.json`; don't duplicate them, just note the caveats below.

- **Website** — `npm run dev` (root) → Next.js on `http://localhost:3000`. To point it at the local Worker instead of production, create `.env.local` (gitignored) with `NEXT_PUBLIC_SERVER_URL=http://localhost:8787` and `NEXT_PUBLIC_SITE_URL=http://localhost:3000`. Without it the site talks to the production API (`app/lib/config.ts`).
- **API** — `cd server && npm run dev` → `wrangler dev` on `http://localhost:8787`. Wrangler uses **local (miniflare) KV/D1/R2 by default — no Cloudflare login needed**; state persists under `server/.wrangler/`. `initEnv` (`worker.ts`) copies string bindings into `process.env`, so `.dev.vars` values reach the code.
- **`server/.dev.vars` is required for local API work and is NOT gitignored (`.env*` doesn't match it) — never commit it.** It needs `ENCRYPTION_KEY` (exactly 64 hex chars, else `crypto.ts` throws), the five integration secrets (`STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `GITHUB_CLIENT_ID`, `GITHUB_CLIENT_SECRET`, `RESEND_API_KEY`), plus `ALLOW_LOCALHOST_CORS=1`, `WEBSITE_URL=http://localhost:3000`, `SERVER_URL=http://localhost:8787`. Placeholder values make `/health` report `env: ok` and unblock everything except the real Stripe/GitHub/Resend/OAuth flows (which need genuine keys).
- **Fresh local D1 is empty — apply the schema once per new `.wrangler` state:** `cd server && npx wrangler d1 migrations apply alexandria-library --local`. Skip this and `/marketplace`, `/library`, and other D1-backed routes return HTTP 500.
- **Smoke test** (`cd server && npm test`, needs the Worker running on :8787): the `Root page returns HTML` check is EXPECTED to fail locally — it asserts the Vercel website's `/` (`<title>Alexandria</title>`), but locally `/` is the Worker's own branded page (`alexandria.`). All other checks pass on a migrated local DB.
- **Lint** (`npm run lint`) runs but has ~29 pre-existing errors (vendored `public/pdf.worker.min.mjs`, `app/start/*` react-hooks rules). CI (`.github/workflows/build.yml`) gates only `tsc --noEmit` (server) + `next build` (root), not eslint.
- **No account/OAuth locally?** Seed a dev Author straight into local KV: build an `Account` JSON (`subscription_status: 'active'`), AES-256-GCM-encrypt it with `ENCRYPTION_KEY` (see `crypto.ts`), and write `account:{github_id}`, `auth:{sha256(apiKey)}`→github_id, `login:{login}`→github_id via `npx wrangler kv key put <k> <v> --binding DATA --local`. Then `Authorization: Bearer alex_…` unlocks `/file`, `/call`, `/marketplace/:module`, `/library/:id/file/:name`. The public `/library` directory is intentionally empty to signed-out strangers, and files default to `authors` visibility (need a key to read).
- **Visual check** — `node scripts/see.mjs localhost --port 3000` (Playwright) needs `npx playwright install chromium` once; screenshots land in `.see/`.
