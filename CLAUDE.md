...
Founder: Benjamin a. Mowinckel. Solo founder + ai agents.

**Canonical surfaces:** `https://alexandria-library.com` (website, Vercel) and `https://api.alexandria-library.com` (API, Cloudflare Worker — protocol, OAuth, billing, Library, cron). `mowinckel.ai` is Benjamin's independent personal website, not the company homepage or a company redirect. Its source is `~/alexandria-inc/public/mowinckel-ai/`; its account, model configuration and published personal context must remain separate from company administration and the company guide. `api.mowinckel.ai` stays bound to the same Worker so legacy CLI/skill installs that cached the old URL keep working; treat it as deprecated. `mcp.mowinckel.ai` is the older alias on the same retire path. Override only via `SERVER_URL` / `NEXT_PUBLIC_SERVER_URL` when intentional.
...
1. **The collective plumbing** (`server/src/protocol.ts` + `auth.ts` + `kv.ts` + `crypto.ts` + `db.ts` + `file-access.ts` + `marketplace-catalog.ts` + `marketplace.ts` + `audit.ts`) — optional server infrastructure for the Library, marketplace, and tribe. Capture, Loop, Skill and the independent Mirror recipe remain free and work without this layer. The paid Connector operates shared discovery, reader identity and current access between independently owned mirrors. Website-only users do not need the private loop, an Alexandria-hosted profile or our model relay. Internally still named `protocol.ts` — a code label for the plumbing, never the public framing.
...
- **Independent websites:** `integration/website-connector/` is the source for the static description helper and optional Web handler. `scripts/package-website.mjs` generates the same-source signed distribution under `factory/website/`; never hand-edit that copy. `factory/canon/connector.md` is the AI's entry module. `setup-connector.sh` prepares only the optional account client; the regular private-loop setup remains separate. Do not route a website-only person through local-loop installation or a hosted profile. Shared service membership gates current company service, never copied public files or own-host inference.
...
- **Storage architecture:** Stateless server; sovereign plain files plus Git history (`~/alexandria/` is the primary live checkout; setup connects no remote or cloud storage); optional user-owned Git remote, iCloud current-file mirror, Drive chat pocket copy, and a bounded Airlock projection behind a separate GitHub account after separate consent; thin persistence for the collective Library (D1 for metadata/discovery, R2 for published content, KV for accounts/events). GitHub is the current soft-default host, never the source of truth. The Airlock account may reach exactly one private app-named repo and host one broad-OAuth experimental AI at a time; private bytes are exact-hash approved and frozen until reapproved, and every return is untrusted capture that never auto-merges into canon.
...
- **The user's ai never sells Alexandria (2026-08-09; active-member opener + completed chat-onboarding carve-outs 2026-08-13; invite helper text removed 2026-09-07).** Alexandria-owned surfaces may carry the pitch. Installed prompts, closes, nudges, and the user's private map never initiate pricing, membership, marketplace, feedback, or other company asks. Two fixed exceptions exist. (1) When the originating setup request asks for it, after the approved private setup and record read-back, give the honest readiness report and exactly `https://alexandria-library.com/join` before optional thinking or extra setup. If saving is unavailable, say so instead of claiming readiness. This neutral requested link is not permission to recommend, browse, connect, share, publish, or use private material to persuade. Run the fullest loop the verified capabilities support; a device or app name is not a capability ceiling. (2) For Authors whose authoritative account state already says they are members, `/a` always opens in the exact order invite → cognitive recommended → everything. The invite body contains only the direct referral URL. `i think you'd like this` is the friend-facing message composed when the Author actually shares the link; it never appears as helper text in the opener. Rendered markdown uses an explicit full-address link; terminals use the plain URL. The model adds no mark, image, emoji, or hand-built preview; host-generated favicon or preview chrome is automatic and accepted. No custom share button or extra share page. Neither exception uses private knowledge to sharpen the company line. Unknown or inactive membership cannot produce an invite. This boundary and the exact carve-outs are release-blocking.
...
- **Pure marginal value add.** Alexandria must never override, compete with, or degrade the user's existing workflows, memory, or tools. Passive mode automatically reads relevant context, preserves the Author's useful contributions and maintains their map within approved access, keeping uncertainty labelled. Routine capture needs no repeated save question; consequential ambiguity, protected-belief changes, new access, sharing and destructive actions retain their gates. Active mode (/a) is opt-in. The user's existing system is the floor — Alexandria only adds.
