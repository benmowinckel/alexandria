...
assert.match(CHAT_INSTRUCTION, /not another assistant/);
assert.match(CHAT_INSTRUCTION, /safest place you can read and update/);
assert.match(CHAT_INSTRUCTION, /private repo I chose for a trusted cloud coding session \(its branch only\)/);
assert.match(CHAT_INSTRUCTION, /Airlock for other remote ai/);
assert.match(CHAT_INSTRUCTION, /this app's memory/);
assert.match(CHAT_INSTRUCTION, /give me a note and say it was not saved/);
assert.match(CHAT_INSTRUCTION, /Choose for me\. Never mix copies/);
assert.match(CHAT_INSTRUCTION, /best unfinished thread/);
assert.match(CHAT_INSTRUCTION, /Save only confirmed changes/);
assert.match(CHAT_INSTRUCTION, /end your first reply with exactly: “Want me to open your alexandria loop/);
