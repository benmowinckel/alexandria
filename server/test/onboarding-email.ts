...
assert.match(agentPrompt, /healthy existing install, preserve it and skip reinstallation/);
...
assert.match(agentPrompt, /Say `start` to continue/);
assert.match(agentPrompt, /Only my subsequent `start` approves the explained setup/);
assert.match(agentPrompt, /Include the AIs I actually use/);
assert.match(agentPrompt, /Do not start a thinking session or delay this completion/);
assert.match(CHAT_INSTRUCTION, /Each chat: read relevant context from my approved record/);
assert.match(CHAT_INSTRUCTION, /Keep its home; if access fails, give an unsaved handoff/);
assert.equal(CHAT_INSTRUCTION, GEMINI_CHAT_INSTRUCTION + '\n\n' + GEMINI_NUDGE_INSTRUCTION);
assert.match(CHAT_SETUP_PROMPT, /Verify in a fresh chat that the instructions load and it reads the record/);
assert.match(CHAT_SETUP_PROMPT, /Adding instructions does not grant another provider access/);
assert.match(CHAT_SETUP_PROMPT, /Keep completed and deferred setup in my approved record/);
assert.match(CHAT_SETUP_PROMPT, /Missing another app must not block/);
assert.doesNotMatch(CHAT_SETUP_PROMPT, /move down the ladder automatically|Then start my first/);
...
assert.ok(CHAT_INSTRUCTION.length <= 1300, `chat instruction lost its headroom: ${CHAT_INSTRUCTION.length}`);
...
assert.match(CHAT_INSTRUCTION, /best approved writable home/);
assert.match(CHAT_INSTRUCTION, /approved writable home: local files/);
assert.match(CHAT_INSTRUCTION, /trusted cloud repo \(committed snapshot, own branch\)/);
assert.match(CHAT_INSTRUCTION, /isolated Airlock; connected storage/);
assert.match(CHAT_INSTRUCTION, /durable app memory/);
assert.doesNotMatch(CHAT_INSTRUCTION, /attached files/);
assert.match(CHAT_INSTRUCTION, /For first setup choose/);
assert.match(CHAT_INSTRUCTION, /best thread/);
assert.match(CHAT_INSTRUCTION, /After yes, save and read back/);
assert.match(CHAT_INSTRUCTION, /each new ordinary conversation, including voice, end only your first reply: “Want me to open your alexandria loop/);
...
assert.match(CHAT_INSTRUCTION, /On yes, open a new chat and invoke the native skill; if unable, tell me the exact start gesture/);
...
assert.match(CHAT_INSTRUCTION, /Report features only from current permissions\/settings\/status/);
...
assert.match(GEMINI_CHAT_INSTRUCTION, /trusted cloud repo \(committed snapshot, own branch\)/);
assert.match(GEMINI_CHAT_INSTRUCTION, /durable app memory/);
assert.match(GEMINI_CHAT_INSTRUCTION, /best approved writable home/);
assert.match(GEMINI_CHAT_INSTRUCTION, /approved writable home: local files/);
assert.match(GEMINI_CHAT_INSTRUCTION, /isolated Airlock; connected storage/);
assert.match(GEMINI_CHAT_INSTRUCTION, /Report features only from current permissions\/settings\/status/);
...
assert.ok(GEMINI_NUDGE_INSTRUCTION.length <= 350, `Gemini first-reply rule lost its headroom: ${GEMINI_NUDGE_INSTRUCTION.length}`);
assert.match(GEMINI_NUDGE_INSTRUCTION, /including voice/);
assert.match(GEMINI_NUDGE_INSTRUCTION, /end only your first reply/);
...
assert.match(CHAT_SETUP_PROMPT, /If this host supports persistent instructions but they are not active, stop and help me fix only that/);
assert.match(CHAT_SETUP_PROMPT, /If it has no persistent instruction setting, keep helping within this chat/);
...
assert.match(CHAT_SETUP_PROMPT, /never silently create a competing record/);
assert.match(CHAT_SETUP_PROMPT, /never call a handoff a saved record/);
assert.match(CHAT_SETUP_PROMPT, /Prepare the highest-value specific thread for my first alexandria session, but do not start it automatically/);
...
assert.equal(chatInstallPrompt('other'), CHAT_INSTRUCTION);
assert.equal(chatSecondaryInstallPrompt('other'), null);
...
assert.match(mechanics, /chat path asks whether you use ChatGPT, Claude, Gemini or another AI, then gives two actions/);
