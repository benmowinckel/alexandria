...
assert.match(CHAT_INSTRUCTION, /safest record you can write and read back/);
assert.match(CHAT_INSTRUCTION, /Claude Code Web: my chosen private GitHub repo, its own branch/);
...
assert.match(GEMINI_CHAT_INSTRUCTION, /a trusted cloud agent's own branch in my approved private GitHub repo/);
assert.match(GEMINI_CHAT_INSTRUCTION, /Gemini memory or an unsaved note/);
assert.match(GEMINI_CHAT_INSTRUCTION, /safest place you can write and read back/);
assert.match(GEMINI_CHAT_INSTRUCTION, /Mac\/Remote Control files/);
assert.match(GEMINI_CHAT_INSTRUCTION, /Airlock or writable connected storage for other remote ai/);
assert.match(GEMINI_CHAT_INSTRUCTION, /A setup default cannot show Library or Airlock is off/);
...
assert.match(localOnboarding, /this setup sent no personal record to Alexandria and connected no account/);
assert.match(localOnboarding, /approved material read into a hosted AI's context is processed by that provider/);
assert.doesNotMatch(localOnboarding, /nothing is sent anywhere|no personal data was shared/);
