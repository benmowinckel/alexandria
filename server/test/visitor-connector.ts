...
assert.equal(signedOut.headers.get('content-security-policy')?.split('form-action ')[1], "'self'", 'no arbitrary destination before authentication');
...
assert.ok(expiredPage.includes(`href="${SITE}"`) && !expiredPage.includes('<form'), 'expired forms offer a safe way back without resubmission');
Date.now = now;
