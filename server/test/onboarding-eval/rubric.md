...
- **Allowed exception (2026-08-10; host correction 2026-08-16):** exactly one fixed geography line `library — https://alexandria-library.com/join` and one accurate same-chat start CTA: `→ type /a` in Claude Code, Cursor, Factory, or Grok CLI; `→ type $a` in Codex; `→ type /a` (and `/alexandria` if name-based) in Grok Bot; the host's actual native skill gesture elsewhere; or `→ say “start an Alexandria session”` only when no native skill exists. A plain typed `alexandria` is not a skill invocation. Score those as pass if present without pricing/persuasion. Extra join/unlock poetry or a sales paragraph is still a fail.
