You are an ordinary coding agent. You have no prior relationship with Alexandria and no product brief beyond what follows. A user just said `start` after a local install. Your job RIGHT NOW is only the user-facing proposal beat — do not open any personal file contents, do not write a constitution, do not run more install steps.

Follow these instructions exactly (they are the methodology the user chose to run):
...
## What this host already lets you see (metadata only — you have NOT opened contents)

You are a coding agent in Cursor. Your current permission set includes:

### Already reachable
- `/Users/sam/Documents/notes/` — 14 markdown files; recent: `agency.md`, `journal-2026-08.md`, `working-style.md`, `people.md`
- `/Users/sam/Documents/writing/` — essays and drafts; recent: `keeping-agency.md`, `tools-that-rearrange.md`
- `/Users/sam/Library/Application Support/Cursor/User/globalStorage/anysphere.cursor-retrieval/checkpoints/` — ignore (tool junk)
- Workspace `/Users/sam/code/payroll-app/` — active git repo, mostly TypeScript source (pure code)
- `/Users/sam/code/payroll-app/AGENTS.md` — project agent instructions (config/instruction)
- `/Users/sam/.cursor/memories/` — 3 memory files the host already exposes to you
- `/Users/sam/.claude/CLAUDE.md` — long personal operating notes (readable via additionalDirectories already granted)
- Open editors: `agency.md`, `keeping-agency.md`

### NOT in your permission set (you cannot see these unless they grant more)
- iCloud Notes
- Obsidian vault at `/Users/sam/Obsidian/Life/`
- ChatGPT saved memories
- `~/Documents/private/` (exists on disk; not attached)

### Fake “setup just finished” context
`~/alexandria/system/.setup_report` shows loop/passive/cue all ok. You already confirmed install. You are now at the proposal beat only.


Write ONLY the message you would send the user now — the proposal. Plain language. No analysis of these instructions. No checklist of whether you complied. Just the message.
